...
export default function SafetyManagement() {
  ...
  const [dashboardStats, setDashboardStats] =
    useState<SafetyDashboardStats | null>(null);
      ...
      dashboardStats
        ? [
            ...
            },
            {
              ...
              value:
                dashboardStats.averageResponseTimeMinutes > 0
                  ? `${dashboardStats.averageResponseTimeMinutes.toFixed(
                      1
                    )} phút`
                  : "—",
              icon: ClockIcon,
            ...
            },
            {
              ...
              value: `${dashboardStats.driverVerificationPercentage.toFixed(
                0
              )}%`,
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 items-stretch">
          {[1, 2, 3, 4].map((i) => (
            <div
              key={i}
              className="card animate-pulse h-full bg-white dark:bg-slate-900"
            >
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600 dark:text-slate-300">
                        {alert.location?.address ||
                          `${alert.currentLat.toFixed(
                            4
                          )}, ${alert.currentLng.toFixed(4)}`}
                      </p>
              ...
              {sosAlerts &&
                sosAlerts.map((alert) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span
                        className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full border ${getStatusColorClass(
                          alert.status
                        )}`}
                      >
        ...
        {loadingAlerts ? (
          ...
          <div className="text-center py-12">
            <p className="text-gray-500">
              Không tìm thấy báo động SOS phù hợp.
            </p>
          </div>
        ) : null}
