...
const computePresetRange = (preset: "today" | "week" | "month") => {
  const now = new Date();
...
const StatCard = memo(({ stat, index }: { stat: any; index: number }) => {
  return (
    ...
    >
      <div
        className={`absolute inset-0 bg-gradient-to-br ${stat.bgGradient} opacity-5`}
      />
...
const ActivityItem = memo(
  ({ activity, index }: { activity: any; index: number }) => {
    const statusColors = {
      ...
      >
        <div
          className={`h-10 w-10 rounded-full flex items-center justify-center text-sm font-semibold text-white ${
            avatarColors[activity.status as keyof typeof avatarColors]
          }`}
        >
          {activity.avatar}
        </div>
...
export default function Dashboard() {
  ...
  const initialRange = useMemo(() => computePresetRange("week"), []);
  ...
  useEffect(() => {
    ...
    loadDashboardData();
  }, [datePreset, dateFrom, dateTo, initialRange.from, initialRange.to]);
  ...
  const stats = useMemo(() => {
    ...
    const avgResponseTime =
      Number(dashboardData.averageResponseTimeMinutes) || 0;
    ...
    const responseTimeChange =
      Number(dashboardData.responseTimeChangeSeconds) || 0;
    ...
    return [
      {
        ...
        change:
          userGrowth !== 0
            ...
            : "0%",
        changeType:
          userGrowth >= 0 ? ("increase" as const) : ("decrease" as const),
        icon: UsersIcon,
      ...
      },
      {
        ...
        change:
          tripGrowth !== 0
            ? `${tripGrowth > 0 ? "+" : ""}${tripGrowth.toFixed(1)}%`
            : "0%",
        changeType:
          tripGrowth >= 0 ? ("increase" as const) : ("decrease" as const),
        icon: Bike,
      ...
      },
      {
        ...
        change:
          revenueGrowth !== 0
            ...
            : "0%",
        changeType:
          revenueGrowth >= 0 ? ("increase" as const) : ("decrease" as const),
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              Số chuyến theo từng khung giờ trong ngày
            </p>
          </div>
          <div className="flex items-center space-x-4">
            <div className="flex items-center space-x-2">
      ...
      >
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              Sự kiện và giao dịch mới nhất trên hệ thống
            </p>
          </div>
          <div className="flex items-center space-x-2">
