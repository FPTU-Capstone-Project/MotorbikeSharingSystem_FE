...
export default function SessionStatus({
  ...
  const { timeUntilExpiry, formattedTime, isTokenValid, isExpiringSoon } =
    useSessionCountdown();
  ...
  return (
    <div className={className}>
      ...
      <div
        className="flex items-center text-sm text-gray-600 dark:text-slate-300"
        style={{ display: "none" }}
      >
        <ClockIcon className="h-4 w-4 mr-1 text-gray-500 dark:text-slate-300" />
        <span>Phiên đăng nhập còn {formattedTime}</span>
      </div>
    </div>
  );
}
